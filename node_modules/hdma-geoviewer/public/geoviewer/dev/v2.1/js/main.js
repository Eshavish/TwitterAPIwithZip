...
var app={
	...
	blacklists:["Longdickya9","vincenvi619","vme7413", "uruguaydiablo", "just_sanch","bhaluPnMC","Sunnyrajkumar1","MoneyOrtega","BixberNavy","genitalia666","EarneyHarisson", "juanesp64250381"],
	user:{},
	icon:{
	...
	streaming:{
		...
		$masonry:$(".searchResult .masonry").masonry({columnWidth:'.masonry-gridSize', itemSelector: 'li.tweetBox'}).on("click",".tweetBox", function(){show.mapPopup($(this))}),
		options:{
...
var init={
	...
	ui:function(){
		...
		$(".section-search").resizable({
			handles:"e",
			minWidth:$mainContent.width()*0.3,
		...
		$(".searchToolbar .dropdown-menu > li").click(function(){
			var $this=$(this),
				$a=$this.find(">a"),
				value=$a.attr("value"),
				type=$a.attr("type");
				...
				if(placename&&placename!=""){
					run.geocoding(placename, function(results, status){
						if(status=='OK'){
							...
							app.layer.queryBoundary=new L.rectangle([[lat-diff, lng-diff], [lat+diff, lng+diff]], {
								stroke: true,
								color: '#f06eaa',
								weight: 4,
								opacity: 0.5,
			...
			if(data){
				...
				if(data.topic){
					...
					if(topic.startDate || topic.endDate){
						...
						if(topic.endDate&&topic.endDate!=""){
	...
	map: function(){		
		app.map = L.map("map", {
		  ...
		  layers: [app.basemaps["ESRI National Geographic Map"].setOpacity(0.7)],
		  zoomControl: false,
		...
		if(t&&t.coordinates){
			...
			if(obj&&obj.marker){
				...
				if(optionsMasonry.isShow){
					...
					$li.data("tweet", t).find(".tweet-mapit").click(function(){
						var $that=$(this);
...
var run={
	...
	//get layers
	getLayers:function(){
		var url="/"+((SERVERNAME=='vision')?"hdma":"ec2")+"/query/getLayer/"+encodeURI(app.topic.name);
	...
	showLayers: function(layers){
		...
		layers.push({
			type:"CARTODB",
			url:"https://giedriusk.cartodb.com/api/v2/viz/1701c998-ee4c-11e4-9cb3-0e8dde98a187/viz.json",
			label:"[Giedrius Kaveckis] Nepal EQ Severity Index Version 4 30 April",
			link:"https://giedriusk.cartodb.com/viz/1701c998-ee4c-11e4-9cb3-0e8dde98a187/public_map",
			description:"It estimates severity based on: 1) the intensity of the earthquake; 2) population; 3) vulnerability of housing and population."
		})
		*/
		//--------------------------------------------
	...
	showLayerInfo: function(data){
		if(data){
			var $target=$('#popup_layerInfo'),
				...
				fields=["type", "url"],
				types={"AGMS":"ArcGIS Map Service", "AGTS":"ArcGIS Tiled Map Service", "AGFS":"ArcGIS Feature Service", "LEAFLET":"Leaflet Feature Layer", "CARTODB":"Cartodb Layer"}
	...
	createLayer: function(obj, callback){
		...
		switch(types[0]){
			case "AGFS":
				featureLayer=new L.esri.FeatureLayer(data.url, {})
				if(callback){callback(featureLayer)}
			break;
			case "AGMS":
				featureLayer=new L.esri.DynamicMapLayer(data.url, {opacity:0.5})
				if(callback){callback(featureLayer)}
			break;
	...
	createTweetWidgetHtml: function(t, options){
		...
		html='<div class="twitter-tweet twitter-tweet-rendered" title="Embedded Tweet" >'+
		        '<div class="github-fork-ribbon-wrapper right"><div class="github-fork-ribbon"><a href="#">Level</a></div></div>'+
					...
					'<blockquote class="tweet subject expanded h-entry" data-tweet-id="'+tweetID+'" cite="https://twitter.com/'+user.screen_name+'/status/'+tweetID+'" >'+
						...
						'<div class="content e-entry-content" data-scribe="component:tweet">'+
							...
							'<div class="dateline collapsible-container">'+
								...
								((t.place&&t.place.full_name)?('<span class="p-geo" data-collapsed-class="collapse-geo" title="Posted from '+t.place.full_name+'"><i class="ic-geo ic-mask"></i><b>'+t.place.full_name+'</b><button class="tweet-mapit btn btn-warning"><i class="fa fa-map-marker"></i>Map it!</button></span>'):"")+
							'</div>'+
						'</div>'+
						/**
